...
### ⚠️ 关键规则
**进入 design 阶段后，先执行知识上下文加载（如 `.claude/knowledge/` 存在），然后立即调用 `EnterPlanMode` 工具。** 知识加载不超过 10 秒。如果 `.claude/knowledge/` 不存在，直接调用 EnterPlanMode。所有的代码探索工作都应该在 Plan Mode 内完成。
...
2. Read `decisions.md` 和 `patterns.md`
3. 判断哪些条目与当前目标相关（同模块、同技术、类似问题）
4. 将相关条目作为内部上下文带入后续 Plan Mode

详细消费规则参见 `references/knowledge-engineering.md`。时间限制 10 秒，最多读取 2 个文件。
...
#### Wave 2 — AI 审查（并行 Agent，基于 Wave 1 结果）
...
4. 有值得记录的条目 → 追加到 `.claude/knowledge/` 对应文件 + 单独 git commit
...
知识文件不属于状态文件，是独立的持久文件。知识提取在 merge 阶段直接写入 `.claude/knowledge/` 目录，用单独的 git commit 提交，不写入状态文件。详细格式和规则参见 `references/knowledge-engineering.md`。
