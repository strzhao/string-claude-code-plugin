...
## 领域 Skill 委托（可选）
> 如果用户目标涉及已有专业 Skill（用户提到了特定 skill 名称，或系统 prompt 中列出的 skill 与目标高度相关），在此声明委托。不声明此字段 = 走默认的蓝/红队对抗路径。
...
当设计文档声明了 `## 领域 Skill 委托` 时，走此路径替代蓝/红队对抗。

**核心理念**：领域 Skill 封装了经过验证的工作流（步骤顺序、工具链约定、资产管理），比蓝队 Agent 从零实现更可靠。

**执行流程**：

1. **调用领域 Skill**
   使用 `Skill: "{skill-name}"` 调用委托 Skill，将设计文档中「委托输入」的信息作为 Skill 参数传递。
   Skill 按自己的内部流程自主执行，编排器不干预 Skill 的内部步骤。

2. **Skill 完成后收集产出**
   - `git status` 确认 Skill 产生的变更文件
   - 记录执行摘要到变更日志

3. **补充验收测试**
   启动红队 Agent 基于设计文档编写验收测试（参考 `references/red-team-prompt.md`）。
   红队的信息隔离原则不变：只看设计文档，不看 Skill 新写的代码。

4. 跳转到 [2. 合流](#2-合流--两个-agent-都完成后)，按正常合流逻辑继续。

**降级策略**：
- Skill 调用失败或不存在 → 变更日志记录失败原因，回退到 [1a. 蓝/红队对抗路径](#1a-蓝红队对抗路径默认) 重新执行
