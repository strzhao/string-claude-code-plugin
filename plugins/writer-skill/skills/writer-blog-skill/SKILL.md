...
- **重要内容不能丢**：原材料里的关键事实、数据、观点、案例，无论怎么改写都必须保留。为了"叙事流畅"而删掉重要信息，是失职
...
- 判断标准：东西真实存在、可以截取的，用 `[截图：描述]`（产品界面、终端输出、聊天记录等）；概念性的、需要示意的，用 `[配图：描述]`（概念图、对比示意等）
- 截图比生成图更有说服力，能截图就截图
- 图片是叙事的一部分，不是装饰。每张图配一句口语化说明，不用"图1："这种标注
- 节奏上大约每 3-5 段一张图，不要连续堆图
