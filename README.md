...
- **功能**：全流程闭环（红蓝对抗 + 五层 QA + 自动修复）+ 智能提交
- **核心特性**：
  - 从目标描述到代码合并全程自动化，仅需两次人工审批
  - 红蓝对抗：蓝队编码 + 红队仅看设计文档写验收测试（信息隔离）
  - 五层 QA + 系统化调试方法论 + 自动修复循环
  - 智能提交：React 优化、代码测验、版本升级、任务同步
  - 防合理化表格 + 成功需要证据原则
  - 知识工程：自动积累项目决策和调试教训
- **技术栈**：Skill + Hook（Stop hook 驱动状态机）
- **作者**：String Zhao
